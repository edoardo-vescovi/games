...
More amazingly, a *proper* implementation on calculator leads to write simple -- yet complete -- examples of dynamic allocation, random-number generation (incl. a cool initalisation), data structures (vectors and hash tables), C++ standard libraries (std::vector and std::unordered_map), classes (incl. the infamous constructor/deconctructor/copy/assignment operators).
