...
This is amazing puzzle for anyone who enjoys mathematical challenges and writing computer programs. Excerpt from a quant researcher job post by Optiver.
...
The appaling simplicity of the model matches only the beautiful need to connect it to disparate results. The mathematical solution encompasses (not so basic elements of) probability, algebraic equations and recursion. The cherry on top is that a naive method *doesn't* always work. The discussion of *why* a solution should exist in the first place will make a connection to a more famous random walk.  

More amazingly, a *proper* implementation on calculator writes simple -- yet complete -- examples of dynamic allocation, random-number generations (and a cool initalisation), data structures (vectors and hash tables), C++ standard libraries (std::vector and std::unordered_map), classes (including the infamous constructor/deconctructor/copy/assignment operators).
