Solution
The ant can explore the half-plane y &le; 1-x. The new challenge is the infinite number of points and variables to solve for. The new food locations have translational symmetry (x,y) &rarr; (x+n,y-n) by any integer n, multiple of 10. This implies the solution has too: E(x,y)=E(x+n,y-n). We can reduce the problem to one dimension by posing E(x,y)=E(x+y). While the ant walks, what matters is whether it moves closer to or away from the food line (the value of x+y), not the movement parallel to the line (the value of x-y). Geometrically, we change coordinates from (x,y) to (x+y,x-y) to make manifest the translational symmetry of the problem.
