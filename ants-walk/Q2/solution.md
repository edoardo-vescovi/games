...
The solution depends on 2 undetermined constants, one of which is fixed by the boundary condition E(10)=0. This delivers E(u) = (c+1)(1-u). The physical constraint E(u) &ge; 0 would lead to conclude that c must be +&#8734;, and so E(u)! Can we prove it rigorously?

Let's introduce a fictitious food location to restrict the ant to the infinite strip -L-x &ge; y &ge; 10-x, with a large integer L, multiple of 10. The recurrence equation, together with the boundary conditions E(10)=E(-L)=0, delivers the *regularised* solution E(u) = (L+1)(1-u). The physics doesn't change though: if we attempt to remove the boundary in the limit L &rarr; &#8734;, then E(u) &rarr; &#8734;.
